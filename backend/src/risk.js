...
exports.updateRiskFromInfer = updateRiskFromInfer;
...
const CRIT_ON = 0.78;
const CRIT_OFF = 0.68;
function updateRiskFromInfer(p) {
    const updated = [];
    for (const z of p.zones) {
        ...
        const prev = state_1.riskByZone.get(id);
        ...
        state_1.riskByZone.set(id, rz);
        updated.push(rz);
    }
    return updated;
}
function transition(prev, risk) {
    if (prev === "normal") {
        if (risk >= CRIT_ON)
            return "critical";
        if (risk >= WARN_ON)
